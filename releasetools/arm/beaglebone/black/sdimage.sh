...
then
# build u-boot
${RELEASETOOLSDIR}/make_uboot.sh -d ${OBJ}/u-boot -c ${UBOOT_CONFIG} -f ${UBOOT_FDT} -t ${UBOOT_CROSS_COMPILE}
else
# build u-boot
${RELEASETOOLSDIR}/make_uboot.sh -d ${OBJ}/u-boot -c ${UBOOT_CONFIG} -f ${UBOOT_FDT} -t ${UBOOT_CROSS_COMPILE}
#      -e <<EOF
...
${RELEASETOOLSDIR}/arm/beaglebone/black/gen_uEnv.txt.sh -c tty00  > ${ROOT_DIR}/uEnv.txt
