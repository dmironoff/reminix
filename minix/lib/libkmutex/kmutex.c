...
* sys_kyield() в цикле проверки мьютекса, что бы дать другим процессам поработать, а самим сохранить свой квант времени
*/
...
void kmutex_lock(kmutex_t *m) {
    while (!arch_kmutex_trylock(m)) {
        for (i = 0; i < KMUTEX_LOCK_SPIN_ITERS; i++) {
             asm volatile ("");
             // Я не стал использовать никакой pause или wfe, так как это погрузит процессор в сон до прерывания
             // А значит время итераций увеличится, нам такое не подходит - мы прогоняем холостые такты и
             // если нет свободного мьютекса уходим в - kyield
        }
        if (!arch_kmutex_trylock(m)) {
            sys_kyield();
        }
    }
};
